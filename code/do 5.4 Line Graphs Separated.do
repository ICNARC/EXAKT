/*******************************************************************************
Project: EXAKT
Do file for: Figures 3 and 4 (Line graphs, measurement error and diagnostic error separately)
*******************************************************************************/
...
twoway 
	...
	,
	by(o, note("") col(5) legend(off) plotregion(margin(l+19)))
	subti("",bc(none))
...
graph combine xaxis visualaxis, col(1) graphregion(col(white)) xcommon imargin(zero) graphregion(margin(l=0 r=0 t=0 b=0)) name(combinedxaxis, replace) nodraw fysize(60) fxsize(160)


******************************* Combined figures *******************************

* main figure for paper
graph combine oximeters bias_92 precision_92 arms_92 /*oneminus_cases_sp92orless oneminus_noncases_sp92orless auc_92 hypox_sp92orless*/ combinedxaxis, col(1) graphregion(col(white)) xcommon imargin(small) graphregion(margin(l=0 r=0)) xsize(20) ysize(20) name(MeasurementCombinedLines, replace) 
//graph save ${OutputDir}\CombinedLines, replace
graph export ${OutputDir}\MeasurementCombinedLines.svg, replace
//graph export ${OutputDir}\CombinedLines.emf, replace
graph export ${OutputDir}\MeasurementCombinedLines.pdf, replace
graph export ${OutputDir}\MeasurementCombinedLines.png, replace
...
graph combine xaxis visualaxis, col(1) graphregion(col(white)) xcommon imargin(zero) graphregion(margin(l=0 r=0 t=0 b=0)) name(combinedxaxis, replace) nodraw fysize(80) fxsize(160)


******************************* Combined figures *******************************

* main figure for paper
graph combine oximeters /*bias_92 precision_92 arms_92*/ oneminus_cases_sp92orless oneminus_noncases_sp92orless auc_92 hypox_sp92orless combinedxaxis, col(1) graphregion(col(white)) xcommon imargin(small) graphregion(margin(l=0 r=0)) xsize(20) ysize(20) name(DiagnosticCombinedLines, replace) 
//graph save ${OutputDir}\CombinedLines, replace
graph export ${OutputDir}\DiagnosticCombinedLines.svg, replace
//graph export ${OutputDir}\CombinedLines.emf, replace
graph export ${OutputDir}\DiagnosticCombinedLines.pdf, replace
graph export ${OutputDir}\DiagnosticCombinedLines.png, replace
